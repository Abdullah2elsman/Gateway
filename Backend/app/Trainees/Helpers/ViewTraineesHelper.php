...
trait ViewTraineesHelper
{
    ...
    {
        // Generate cache key based on user, permissions, and filters
        $cacheKey = sprintf(
            'trainees_%s_%s_%s_%s',
            $class->permission_collection,
            $class->current_user->id,
            $request->filled('branch') ? $request->branch : 'all',
            md5(serialize($class->current_user->permissions ?? []))
        );

        // Cache the results for 5 minutes
        $trainees_data = \Illuminate\Support\Facades\Cache::remember($cacheKey, 300, function () use ($trainees, $request, $class) {
            $data = [];
