...
{
    use TraineeDataHelper, UpdateTraineeEssentialData, UpdateTraineeAddtionalData, 
    GetBranch, GetList, CreateMeta, UpdateMeta, PermissionUniqueness, GetUser, 
    CheckPermissionByBranch, SendNotification;

    public function __construct(?Trainee $trainee, $current_user, $id)
    {
        Gate::authorize('updateRefundTrainee', $trainee->find($id));
    ...
    public function update(?Trainee $trainee, ?TraineeMeta $TraineeMeta, Request $request, $id)
    {
        try 
        {
            ...
            $this->notifyUser('has updated a trainee on the refund list', $this->current_user, 'update_trainees_in_refundlist');
