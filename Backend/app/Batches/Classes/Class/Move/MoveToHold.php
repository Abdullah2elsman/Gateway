...
class MoveToHold
{
    ...
    public function moveToHold(?Trainee $trainees, ?TraineeClass $trainee_class, $class_id, $trainee_id)
    {
        try
        {
            ...
            if(($trainee->current_list !== $this->List($this->list)->id) && $is_exists && $batch_is_active)
            {
                ...
                $this->notifyUser('has moved a trainee from a class to hold list', $this->current_user, 'move_from_class_to_hold');
