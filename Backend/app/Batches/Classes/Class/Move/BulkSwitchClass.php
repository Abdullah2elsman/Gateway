...
class BulkSwitchClass
{
    ...
    public function switchClass(?Classes $class, ?TraineeClass $trainee_class, Request $request)
    {
        try
        {
            $message = response(['message' => "Cannot switch class. Class not exists or class id is wrong."], 400);
            ...
            foreach($request->trainees as $trainee_id)
            {
                ...
                if(!$is_exists && $request->filled('old_class') && boolval($batch_is_active))
                {
                    ...
                    $message = response(['message' => "Class switched successfully."], 200);
                }
            }

            $this->notifyUser('has switched trainees class', $this->current_user, 'switch_class');
