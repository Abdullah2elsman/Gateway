...
class ViewSingleTrainee
{
    ...
    public function viewSingleTraineeData(?Trainee $trainee, Request $request, $trainee_id)
    {
        try
        {
            ...
            $cacheKey = 'trainee_single_' . $trainee_id . '_user_' . $this->current_user->id . $branchFilter . '_' . ($isBranchRestricted ? 'restricted' : 'all');

            return Cache::remember($cacheKey, 300, function () use ($trainee, $request, $trainee_id, $isBranchRestricted) {
