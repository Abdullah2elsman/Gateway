...
class ViewTraineeData
{
    ...
    public function viewTraineeData(?Trainee $trainee, Request $request)
    {
        try {
            ...
            $allLists = \App\Models\GtList::pluck('list_title', 'id')->toArray();
            $allGeneralMeta = \App\Models\GeneralMeta::pluck('meta_value', 'id')->toArray();
            ...
            foreach ($traineeClasses as $traineeId => $classes) {
                ...
                if ($latestClass && $latestClass->classes) {
                    $traineeClassesMap[$traineeId] = $latestClass->classes;
                }
            }
