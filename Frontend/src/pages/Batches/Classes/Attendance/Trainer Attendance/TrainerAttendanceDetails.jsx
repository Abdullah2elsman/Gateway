...
const TrainerAttendanceDetails = () => {
  ...
  const { trainer_attendance, error, isLoading } = useSelector(
  ...
  useEffect(() => {
    if (trainer_attendance?.attendances) {
      setAttendances(
        trainer_attendance?.attendances?.find(
          (attendance) => attendance.id === state.id
        )
      );
    }
  }, [trainer_attendance, state]);
  ...
  return (
    <div className={styles.Details}>
      <Helmet>
        <meta charSet="utf-8" />
        <title>{`${context} | Trainer Attendance`}</title>
      </Helmet>
      ...
      <div className={styles.details_content}>
        ...
        <div className={styles.Batch_title}>
          <h3>{attendances?.class_name}</h3>
          {attendances?.gate_url && attendances?.gate_password && (
            <div className={styles.Batch_title} style={{ paddingTop: "20px" }}>
              ...
              >
                ...
                <p>
                  <strong>Password: </strong> {attendances?.gate_password}
                </p>
              </div>
            </div>
          )}
        ...
        <div className={styles.content_wapper}>
          ...
          <div className={styles.content_info}>
            ...
            <p>{attendances?.num_of_trainees}</p>
          </div>
          ...
          <div className={styles.content_info}>
            ...
            <p>{attendances?.num_of_confirmation}</p>
          </div>
        </div>
      </div>
      ...
      <div className={styles.trainees}>
        ...
        <div className={styles.table}>
          <AdvancedTable
            ...
            rows={(!attendances?.message && attendances?.trainees) || []}
            isLoading={isLoading}
...
export default TrainerAttendanceDetails;
